...
function edds_preapproved_payment_needs_action_notification( $payment_id ) {
	...
	$subject = esc_html__( 'Your Preapproved Payment Requires Action', 'edds' );
	$heading = edd_do_email_tags( esc_html__( 'Payment Requires Action', 'edds' ), $payment_id );

	$message  = esc_html__( 'Dear {name},', 'edds' ) . "\n\n";
	$message .= esc_html__( 'Your preapproved payment requires further action before your purchase can be completed. Please click the link below to take finalize your purchase', 'edds' ) . "\n\n";
