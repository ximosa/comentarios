			...
			case 'charge.refunded':
				...
				if ( $payment && $payment->ID > 0 ) {
					...
					if ( $charge->refunded ) {
						...
						$payment->add_note( sprintf( __( 'Charge %s has been fully refunded in Stripe.', 'edds' ), $charge->id ) );
						...
						$payment->add_note( sprintf( __( 'Charge %s partially refunded in Stripe.', 'edds' ), $charge->id ) );
					}
				}
				...
				if ( $payment && $payment->ID > 0 ) {
					$payment->add_note(
						sprintf(
							...
							__( 'Stripe Radar review opened with a reason of %s.', 'edds' ),
							$review->reason
						)
					);
					$payment->save();
				...
				if ( $payment && $payment->ID > 0 ) {
					$payment->add_note(
						sprintf(
							...
							__( 'Stripe Radar review closed with a reason of %s.', 'edds' ),
							$review->reason
						)
					);
					$payment->save();
