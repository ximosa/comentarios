...
function edds_prb_shim_gateways( $gateways ) {
	...
	$gateways = array_merge(
		array(
			'stripe-prb' => array(
				'admin_label'    => __( 'Express Checkout (Apple Pay/Google Pay)', 'edds' ),
				'checkout_label' => __( 'Express Checkout', 'edds' ),
				'supports'       => array(),
			),
		),
		$gateways
...
function edds_prb_purchase_form_required_fields() {
	return array(
		'edd_email' => array(
			...
			'error_message' => __( 'Please enter a valid email address', 'edds' )
		),
	);
}
...
function edds_prb_load_gateway() {
	if ( ! isset( $_POST['nonce'] ) ) {
		edd_debug_log(
			__( 'Missing nonce when loading the gateway fields. Please read the following for more information: https://easydigitaldownloads.com/development/2018/07/05/important-update-to-ajax-requests-in-easy-digital-downloads-2-9-4', 'edds' ),
			true
		);
	}
